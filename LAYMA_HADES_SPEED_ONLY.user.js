// ==UserScript==
// @name         LAYMA + HADES SPEED (FIXED)
// @namespace    http://tampermonkey.net/
// @version      6.0
// @description  Bypass captcha từ layma.net (xử lý HTML)
// @author       @tpmodz
// @match        *://*/*
...
(function() {
    ...
    const _ct = window.clearTimeout, _ci = window.clearInterval;
    ...
    function showHades() {
        ...
        c.id = id;
        c.style.cssText = `
            position:fixed; top:25px; right:25px; background:rgba(30,35,50,0.9);
            color:#e0e0e0; padding:12px 18px; border-radius:12px; z-index:2147483647;
            ...
            transform:translateX(120%); transition:all 0.5s cubic-bezier(0.25,1,0.5,1);
        `;
        c.innerHTML = `
            <span style="font-size:24px;">⚡</span>
            <div><div style="font-weight:700;color:#fff;">HadesSpeed x${SPEED}</div>
            <div style="font-size:13px;color:#b0b8c8;">Bypass đang chạy siêu tốc</div></div>
        ...
        c.querySelector('.hades-close').addEventListener('click', () => {
            c.classList.remove('visible');
            setTimeout(() => c.remove(), 500);
            hadesShown = false;
        });
        setTimeout(() => {
            if (c.parentNode) { c.classList.remove('visible'); setTimeout(() => c.remove(), 500); hadesShown = false; }
        }, 8000);
    }

    // =============================================================
    //  LAYMA UI (rút gọn nhưng đầy đủ)
    // =============================================================
    ...
    style.textContent = `
        ...
        #lm-root {
            ...
            backdrop-filter:blur(18px); border:1px solid rgba(56,189,248,0.25);
            border-radius:18px; box-shadow:0 24px 70px rgba(0,0,0,0.85);
            font-family:'Segoe UI',system-ui,sans-serif; color:#e8edf5; user-select:none;
        }
        #lm-header {
        ...
        #lm-bar-fill {
            ...
            background-size:300% 100%; transition:width 0.5s cubic-bezier(0.4,0,0.2,1);
    ...
    // =============================================================
    //  CAPTCHA FLOW – XỬ LÝ HTML
    // =============================================================
    function fetchCaptcha(host) {
        return new Promise((resolve, reject) => {
            const url = 'https://layma.net/Traffic/Index/?host=' + encodeURIComponent(host);
            log('📡 Fetching captcha from layma...', 'info');
            setProgress(20);
            ...
            GM_xmlhttpRequest({
                ...
                onload: function(res) {
                    ...
                    // Tìm captcha_id trong HTML
                    let captchaId = null;
                    let image = null;

                    // Tìm bằng regex
                    const idMatch = html.match(/captcha_id["']?\s*[:=]\s*["']?([a-f0-9]{32,})/i);
                    if (idMatch) captchaId = idMatch[1];

                    // Nếu không tìm thấy, thử tìm trong script hoặc input
                    if (!captchaId) {
                        const inputMatch = html.match(/<input[^>]*name=["']captcha_id["'][^>]*value=["']([^"']+)["']/i);
                        if (inputMatch) captchaId = inputMatch[1];
                    }
                    ...
                    if (captchaId) {
                        ...
                        setProgress(40);
                        resolve({ captcha_id: captchaId, image: image });
                    } else {
                        reject(new Error('Không tìm thấy captcha_id trong HTML'));
                    }
                },
                onerror: reject,
    ...
    function solveCaptcha(captchaId) {
        return new Promise((resolve, reject) => {
            const url = 'https://layma.net/Traffic/Solve/';
            ...
            GM_xmlhttpRequest({
                method: 'POST',
                url: url,
                data: 'captcha_id=' + encodeURIComponent(captchaId),
                ...
                onload: function(res) {
                    ...
                    // Tìm code trong HTML
                    let code = null;
                    const codeMatch = html.match(/code["']?\s*[:=]\s*["']?([a-zA-Z0-9]+)["']?/i);
                    if (codeMatch) code = codeMatch[1];

                    // Hoặc tìm trong div chứa code
                    if (!code) {
                        const divMatch = html.match(/<div[^>]*id=["']code["'][^>]*>(.*?)<\/div>/i);
                        if (divMatch) code = divMatch[1].trim();
                    }
                    ...
                    if (code) {
                        ...
                        reject(new Error('Không tìm thấy code trong response'));
                    }
                },
                onerror: reject,
                ontimeout: () => reject(new Error('Timeout'))
    ...
    async function startBypass() {
        ...
        try {
            ...
            if (data.image) {
                captchaTarget.innerHTML = `<img src="${data.image}" />`;
    ...
    log('🔥 LAYMA + HADES (HTML mode) ready.', 'info');

...
(function () {
  ...
  if (typeof GM_registerMenuCommand !== "undefined") {
    GM_registerMenuCommand("▶ Run Bypass", function () {
      var root = document.getElementById("utl-root");
      if (!root) {
        initUI();
    ...
    GM_registerMenuCommand("👁 Show / Hide Panel", function () {
      var root = document.getElementById("utl-root");
      if (!root) { initUI(); return; }
      root.style.display = root.style.display === "none" ? "" : "none";
    });

    GM_registerMenuCommand("🔄 Reset & Reload", function () {
      if (typeof GM_setValue !== "undefined") {
        GM_setValue("vxrn-saved_domain", "{}");
      } else {
        localStorage.removeItem("vxrn-saved_domain");
      }
      location.reload();
    });
  }

...
async function solveIconCaptcha() {
    ...
    // Tìm ảnh captcha bị nhiễu — ưu tiên src có captcha/icon
    const allImgs = Array.from(document.querySelectorAll('img'));
    const captchaImg = allImgs.find(img => {
        const src = img.src || '';
        return src.includes('captcha') || src.includes('icon') || src.includes('challenge');
    }) || allImgs.find(img => img.width > 20 && img.width < 200 && img.height > 20);

    if (!captchaImg) { log('[WARN] Không tìm thấy ảnh captcha'); return false; }
    log('Captcha img src: ' + captchaImg.src.substring(0,80));

    // Luôn dùng GM_xmlhttpRequest để tránh CORS
    let base64;
    try { base64 = await urlToBase64(captchaImg.src); }
    catch(e) { log('[ERROR] Không tải được ảnh: ' + e); return false; }

    if (!base64 || base64.length < 100) { log('[ERROR] Base64 rỗng'); return false; }
    log('Base64 length: ' + base64.length);

    // Tìm SVG icons của 5 lựa chọn — lấy outerHTML
    const optionEls = document.querySelectorAll('.captcha-option, [class*="captcha"] button, [class*="captcha"] [class*="option"]');
    let optionsDesc = '';
    if (optionEls.length > 0) {
        optionsDesc = `\n5 LỰA CHỌN (thứ tự 1-5 từ trái sang phải, hàng trên 4 cái, hàng dưới 1 cái giữa):\n`;
        optionEls.forEach((el, i) => {
            const svg = el.querySelector('svg');
            const use = svg ? svg.querySelector('use') : null;
            const href = use ? (use.getAttribute('href') || use.getAttribute('xlink:href') || '') : '';
            const iconName = href.replace(/.*#/, '') || el.className || `option${i+1}`;
            optionsDesc += `${i+1}. ${iconName}\n`;
        });
    }
    const optionsText = optionsDesc;

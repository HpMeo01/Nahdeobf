...
(function () {
  ...
  const _0x4f12 = 'https://kiemgao.site/dashboard/tasks', _0x3b8a = document.body ? document.body.innerText.toLowerCase() : '';
  ...
  // ERROR CODE CHECK — redirect về homepage nếu path match mã lỗi
  (function() {
    const _0xerrCodes = (GM_getValue('errorCodes', '')).split('\n').map(s => s.trim()).filter(Boolean);
    const _0xhomepage = GM_getValue('homepage', '');
    if (_0xerrCodes.length > 0 && _0xhomepage) {
      const _0xcurPath = window.location.pathname.replace(/^\/|\/$/g, ''); // strip leading/trailing slash
      for (let _0xec of _0xerrCodes) {
        const _0xecClean = _0xec.replace(/^\/|\/$/g, '');
        if (_0xcurPath === _0xecClean || _0xcurPath.startsWith(_0xecClean + '/') || _0xcurPath.endsWith('/' + _0xecClean)) {
          window.location.href = _0xhomepage; return;
        }
      }
    }
  })();
  // GITHUB CONFIG
  const _0xlinkToken = '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' // Token for link.json
  const _0xlinkRepo = 'Donam-da/bypass' // Repo for link.json
  const _0xlinkFile = 'link.json' // link.json
  ...
  _0xst.innerHTML = `@keyframes settingsFadeIn { from { opacity:0; transform:scale(0.95); } to { opacity:1; transform:scale(1); } } @keyframes slideIn { from { opacity: 0; transform: translateX(-10px); } to { opacity: 1; transform: translateX(0); } } @keyframes bounceSticker { 0%, 100% { transform: translateY(0) rotate(0deg); } 50% { transform: translateY(-8px) rotate(5deg); } } @keyframes pulseGlow { 0% { box-shadow: 0 0 15px rgba(168, 85, 247, 0.4); } 50% { box-shadow: 0 0 25px rgba(168, 85, 247, 0.7); } 100% { box-shadow: 0 0 15px rgba(168, 85, 247, 0.4); } } .lux-panel { position: fixed; bottom: 30px; right: 30px; width: 390px; height: 350px; background: rgba(10, 10, 10, 0.95); backdrop-filter: blur(15px); -webkit-backdrop-filter: blur(15px); border: 1px solid rgba(192, 132, 252, 0.3); border-radius: 16px; z-index: 2147483647; animation: pulseGlow 3s infinite; display: flex; flex-direction: column; overflow: visible; font-family: 'Segoe UI', system-ui, sans-serif; transition: height 0.3s cubic-bezier(0.4, 0, 0.2, 1); } .lux-header { background: rgba(0, 0, 0, 0.5); color: #f0f0f0; padding: 14px 18px; font-size: 14px; font-weight: bold; border-bottom: 1px solid rgba(255, 255, 255, 0.1); border-radius: 16px 16px 0 0; display: flex; justify-content: space-between; align-items: center; user-select: none; } .lux-body { flex-grow: 1; padding: 16px; overflow-y: auto; line-height: 1.6; font-size: 13.5px; transition: opacity 0.2s ease; } .lux-body::-webkit-scrollbar { width: 6px; } .lux-body::-webkit-scrollbar-thumb { background: rgba(192, 132, 252, 0.5); border-radius: 10px; } .log-entry { animation: slideIn 0.3s ease forwards; margin-bottom: 8px; padding: 8px 10px; background: rgba(255,255,255,0.05); border-radius: 8px; border-left: 3px solid transparent; display: flex; align-items: flex-start; } .log-icon { margin-right: 8px; font-size: 16px; } .log-text { color: #e2e8f0; font-family: 'Consolas', monospace; letter-spacing: 0.2px; font-weight: 500;} .lux-btn { background: none; border: none; color: #c084fc; cursor: pointer; font-size: 20px; font-weight:bold; margin-left: 10px; transition: color 0.2s; padding: 0 5px;} .lux-btn:hover { color: #fff; } .arh-sticker { position: absolute; top: -35px; left: 20px; animation: bounceSticker 2s infinite ease-in-out; box-shadow: 0 5px 15px rgba(0,0,0,0.5); z-index: 2147483648; pointer-events: none;} .lux-settings-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.6); z-index: 2147483646; display: flex; align-items: center; justify-content: center; } .lux-settings-box { background: rgba(15,15,15,0.98); border: 1px solid rgba(192,132,252,0.4); border-radius: 14px; padding: 24px; width: 360px; max-width: 90vw; animation: settingsFadeIn 0.2s ease forwards; font-family: 'Segoe UI', system-ui, sans-serif; } .lux-settings-box h3 { color: #c084fc; margin: 0 0 18px; font-size: 15px; letter-spacing: 0.5px; display: flex; align-items: center; gap: 8px; } .lux-settings-box label { display: block; color: #94a3b8; font-size: 12px; margin-bottom: 6px; margin-top: 14px; letter-spacing: 0.3px; } .lux-settings-box input, .lux-settings-box textarea { width: 100%; box-sizing: border-box; background: rgba(0,0,0,0.5); border: 1px solid rgba(192,132,252,0.25); border-radius: 6px; color: #e2e8f0; font-family: 'Consolas', monospace; font-size: 12.5px; padding: 8px 10px; outline: none; resize: vertical; } .lux-settings-box input:focus, .lux-settings-box textarea:focus { border-color: rgba(192,132,252,0.6); } .lux-settings-actions { display: flex; gap: 10px; margin-top: 18px; } .lux-save-btn { flex: 1; padding: 9px; background: linear-gradient(135deg, #c084fc, #a855f7); color: #fff; border: none; border-radius: 6px; cursor: pointer; font-weight: bold; font-size: 13px; } .lux-cancel-btn { padding: 9px 16px; background: rgba(255,255,255,0.07); color: #94a3b8; border: 1px solid rgba(255,255,255,0.1); border-radius: 6px; cursor: pointer; font-size: 13px; }`;
  document.head.appendChild(_0xst);
  let _0xP = document.createElement('div'); _0xP.className = 'lux-panel';
  let _0xS = document.createElement('img'); _0xS.className = 'arh-sticker'; _0xS.src = 'https://cdn.phototourl.com/free/2026-06-14-6f638068-8681-4583-a107-37d6bacd3a80.jpg'; _0xS.style.width = '55px'; _0xS.style.height = '55px'; _0xS.style.borderRadius = '50%'; _0xS.style.objectFit = 'cover'; _0xS.style.border = '2px solid rgba(192, 132, 252, 0.8)'; _0xP.appendChild(_0xS);
  let _0xH = document.createElement('div'); _0xH.className = 'lux-header'; _0xH.innerHTML = `<div style="display:flex; align-items:center; gap: 8px;"><span style="display:inline-block; width:10px; height:10px; background:#4ade80; border-radius:50%; box-shadow: 0 0 8px #4ade80;"></span><span style="letter-spacing: 0.5px; text-shadow: 0 2px 4px rgba(0,0,0,0.5);">未知用户 TỰ ĐỘNG</span></div><div style="display:flex;align-items:center;gap:4px;"><button id="lux-settings-btn" class="lux-btn" title="Cài đặt" style="font-size:16px;">⚙</button><button id="lux-toggle-btn" class="lux-btn" title="Thu nhỏ / Phóng to">−</button></div>`; _0xP.appendChild(_0xH);
  ...
  function openSettings() {
    if (document.getElementById('lux-settings-overlay')) return;
    let _0xov = document.createElement('div'); _0xov.className = 'lux-settings-overlay'; _0xov.id = 'lux-settings-overlay';
    _0xov.innerHTML = `
      <div class="lux-settings-box">
        <h3>⚙ Cài đặt hệ thống</h3>
        <label>🏠 Trang chủ (URL redirect khi gặp mã lỗi)</label>
        <input type="text" id="cfg-homepage" placeholder="https://kiemgao.site/dashboard/tasks" value="${GM_getValue('homepage', '')}">
        <label>🚫 Mã lỗi (mỗi dòng 1 mã, VD: 220-2)</label>
        <textarea id="cfg-errcodes" rows="5" placeholder="220-2\n150-1\n99-3">${GM_getValue('errorCodes', '')}</textarea>
        <div class="lux-settings-actions">
          <button class="lux-save-btn" id="cfg-save-btn">💾 Lưu</button>
          <button class="lux-cancel-btn" id="cfg-cancel-btn">Huỷ</button>
        </div>
      </div>`;
    document.body.appendChild(_0xov);
    document.getElementById('cfg-cancel-btn').addEventListener('click', () => _0xov.remove());
    _0xov.addEventListener('click', (e) => { if (e.target === _0xov) _0xov.remove(); });
    document.getElementById('cfg-save-btn').addEventListener('click', () => {
      let _0xhp = document.getElementById('cfg-homepage').value.trim();
      let _0xec = document.getElementById('cfg-errcodes').value.trim();
      GM_setValue('homepage', _0xhp);
      GM_setValue('errorCodes', _0xec);
      _0xov.remove();
      logG('Đã lưu cài đặt thành công.', 'success');
    });
  }
  document.getElementById('lux-settings-btn').addEventListener('click', openSettings);
  ...
  function startJ(_0xrdV, _0xuJ, _0xtr = 0, _0xsrcJ) {
    ...
    let _0xhds = { accept: '*/*', ['content-type']: 'application/x-www-form-urlencoded', ['content-value-random']: _0xrdV, origin: _0xhJ, referer: _0xuJ, ['user-agent']: _0xua };
    ...
    GM_xmlhttpRequest({
      ...
      onload: function (_0xrJob) {
        let _0xjj; try { _0xjj = JSON.parse(_0xrJob.responseText); } catch (e) { return setTimeout(() => startJ(_0xrdV, _0xuJ, _0xtr + 1, _0xsrcJ), 3000); }
        if (_0xjj.status !== 'success') return setTimeout(() => startJ(_0xrdV, _0xuJ, _0xtr + 1, _0xsrcJ), 3000);
      ...
      }, onerror: function () { setTimeout(() => startJ(_0xrdV, _0xuJ, _0xtr + 1, _0xsrcJ), 3000); }, ontimeout: function () { setTimeout(() => startJ(_0xrdV, _0xuJ, _0xtr + 1, _0xsrcJ), 3000); }
    });
  }
  function contJ(_0xrdV, _0xuJ, _0xpay, _0xhds, _0xst, _0xtr = 0) {
    if (_0xtr > 3) return logG('Phát sinh lỗi kết nối đa điểm. Tạm dừng tiến trình.', 'error');
    GM_xmlhttpRequest({
      method: 'POST', url: 'https://octolink.vip/check/continue', data: _0xpay, headers: _0xhds, timeout: 60000,
      onload: function (_0xrC) {
        let _0xjc; try { _0xjc = JSON.parse(_0xrC.responseText); } catch (e) { return setTimeout(() => contJ(_0xrdV, _0xuJ, _0xpay, _0xhds, _0xst, _0xtr + 1), 3000); }
        ...
        } else { if (_0xjc.status === 'success') { logG(`Hoàn tất chặng ${_0xst}, tiếp tục di chuyển...`, 'success'); setTimeout(() => { startJ(_0xrdV, _0xuJ, 0, 'cache'); }, 1000); } else { setTimeout(() => contJ(_0xrdV, _0xuJ, _0xpay, _0xhds, _0xst, _0xtr + 1), 3000); } }
